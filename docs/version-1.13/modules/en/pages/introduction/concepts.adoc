...
If both data engines are enabled on a cluster, each node runs two Instance Manager pods (one per data engine version), each with its own CPU reservation. {longhorn-product-name} does not replace an Instance Manager pod while it still hosts engine or replica instances; how instances reach the upgraded pod differs between the two data engines.

===== V1 Data Engine

During a {longhorn-product-name} upgrade, a new Instance Manager pod is created alongside the existing one on each affected node.

* *Coexistence*: The old pod keeps hosting the engine and replica instances that are already running so that live volumes stay online, while newly created and newly attached volumes land on the upgraded pod.
* *Transition*: Existing volumes only move to the new Instance Manager when they are detached and reattached (typically as part of the xref:upgrades/longhorn-components/manually-upgrade-engine.adoc[engine upgrade workflow]).
* *Cleanup*: The old pod is only removed once no instances remain inside it. Old pods that are still running after the upgrade are therefore expected; see xref:upgrades/longhorn-components/instance-manager-pods-during-upgrade.adoc[Instance Manager Pods During Upgrade].

===== V2 Data Engine

Old and new pods cannot coexist on the same node, because the SPDK target process takes ownership of the node's block-type disks and uses the pod's dedicated CPU cores and, when enabled, hugepages. The V2 Instance Manager pod is therefore replaced in place with the new image instead of being started alongside the old one. During a V2 Instance Manager live upgrade, {longhorn-product-name} handles the upgrade as follows:

* *No active instances*: If the V2 Instance Manager on a node hosts no engine or replica instances (for example, the node has no attached V2 volumes and no V2 replicas scheduled on it), {longhorn-product-name} replaces the pod as soon as a new image is available.
* *Active instances (Live upgrade)*: If instances are still running, {longhorn-product-name} supports a live upgrade when upgrading from v1.12.2 or later. This keeps volumes attached throughout the upgrade. {longhorn-product-name} upgrades one node at a time: it temporarily relocates the engines hosted on that node to other nodes, detaches the replicas on the node from their engines, replaces the Instance Manager pod, restores the engines to their original node, and waits for the affected volumes to become healthy again before moving on. Workloads and the host-side NVMe-TCP initiators stay on their original nodes throughout.
* *Offline fallback*: Live upgrade has prerequisites. In particular, live upgrade is not supported in a single-node cluster. The cluster must have more than one node, use the NVMe-TCP frontend, and have healthy replicas on at least two nodes. If these prerequisites are not met, detach the V2 volumes so that each Instance Manager can be replaced with no instances running.

[WARNING]
====
*Resource constraints during upgrades*: 

For the V1 Data Engine, both the old and new pods keep their CPU and memory reservations until the old pod is removed, so each node needs enough spare capacity to run the extra pod. If a node does not have enough reservable resources, {longhorn-product-name} cannot start the new Instance Manager pod and the upgrade stalls on that node.

For the V2 Data Engine, the pod is replaced in place, so no extra hugepage or CPU reservation is needed on the node being upgraded. A live upgrade instead shifts load elsewhere: the relocated engines temporarily add I/O to the nodes that host them, and the replicas that were detached during the upgrade may have to be rebuilt afterwards, which requires spare disk capacity.
====
