...
. I/O flows from the application, through the filesystem and *iSCSI block device*, to the *`longhorn tgt` service*.
. The service forwards the I/O to the *Longhorn Engine process* over a Unix domain socket using a customized protocol.
. The Engine forwards the I/O to *Longhorn replica processes* on multiple nodes over TCP.
. Each replica process stores the volume data in a sparse file on the filesystem.
...
image::diagrams/architecture/v1-data-path.png[V1 Data Engine data path from the application to Longhorn replica processes]

==== 1.2.2. V2 Data Engine

For V2 volumes, I/O follows this path:

. I/O flows from the application, through the filesystem and *NVMe-oF block device*, to the *SPDK target daemon* (which exposes the Longhorn Engine instance as an SPDK RAID1 block device).
. The Engine instance forwards the I/O to the SPDK target daemons on multiple nodes over NVMe-oF using TCP.
. Each replica instance acts as an SPDK logical volume that stores the volume data on a raw block device.
...
image::diagrams/architecture/v2-data-path.png[V2 Data Engine data path from the application to Longhorn replica instances]
