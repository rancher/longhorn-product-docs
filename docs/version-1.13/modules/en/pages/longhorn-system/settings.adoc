...
This is an installation-time setting and cannot be changed after {longhorn-product-name} is initialized.

This setting can be used with `Create Default Disk on Labeled Nodes` to make {longhorn-product-name} use only nodes that have specific storage mounted at, for example, `/opt/longhorn`, when scaling the cluster.
