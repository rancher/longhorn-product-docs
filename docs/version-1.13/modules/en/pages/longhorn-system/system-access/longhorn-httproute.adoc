...
| bool
| `false`
| Enables HTTPRoute generation for the {longhorn-product-name} UI.
...
| list
| `[]`
| List of Gateway API filters to attach to the HTTPRoute rule for the {longhorn-product-name} UI.

| `httproute.path`
| string
| `"/"`
| The routing path used to access the {longhorn-product-name} UI.
...
=== Secure access to the {longhorn-product-name} UI with basic auth integration

To enable authenticated access to the {longhorn-product-name} UI, reference an existing authentication proxy or middleware resource through the HTTPRoute `filters` field.
...
Install {longhorn-product-name} using the same values file:
