...
* How the backups or snapshots are retained, either by number (`count-based`) or by age (`age-based`). For more information, see xref:_retention_policies.adoc[Retention Policies].
...
[NOTE]
====
Retain value has no effect for this task, {longhorn-product-name} automatically mutates the `retain` value to 0 and the `retainAge` value to `0s`
====
...
[NOTE]
====
The `snapshot-cleanup`, `snapshot-delete`, and `filesystem-trim` tasks support only the `count-based` retention policy. Longhorn rejects such a recurring job when `retentionPolicy` is set to `age-based`.
====
...
* Recurring jobs created before Longhorn v{patch-version} use `count-based` retention with their existing `retain` value after the upgrade, so their behavior is unchanged.
====
