...
The `longhornio/longhorn-instance-manager:v1.11.0` image is affected by regressions introduced by recent changes in the instance manager:

* Proxy service API regression causing connection leaks and increased memory usage https://github.com/longhorn/longhorn/issues/12573[Issue #12573]
* V2 data engine block-type disk check regression https://github.com/longhorn/longhorn/issues/12599[Issue #12599]

These issues are resolved in hotfix images. If you are running version `v1.11.0`, update the image to `longhornio/longhorn-instance-manager:v1.11.0-hotfix-2`. This image contains fixes for both regressions.

The earlier hotfix image, `longhornio/longhorn-instance-manager:v1.11.0-hotfix-1`, only includes the fix for the proxy connection leak. Users can choose to apply either hotfix image based on their needs.

To apply the update:

. *Update the `longhorn-instance-manager` image*.
+
Change the image tag from `v1.11.0` to `v1.11.0-hotfix-2` (or `v1.11.0-hotfix-1`) in the configuration file:
+
...
The `longhorn-manager:v1.11.0` image is affected by a https://github.com/longhorn/longhorn/issues/12578[regression issue] introduced by the `Kubernetes Node` validator. This bug prevents setting Kubernetes node CNI labels because the validator waits for the {longhorn-product-name} webhook server to run, while the webhook server waits for the CNI network to be ready.

To mitigate this issue, replace `longhornio/longhorn-manager:v1.11.0` with the hotfix image `longhornio/longhorn-manager:v1.11.0-hotfix-1`.

To apply the update:

. *Disable the upgrade version check*.
** For Helm - Set `upgradeVersionCheck` to `false` in `values.yaml`.
